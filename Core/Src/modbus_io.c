...
uint16_t modbus_io_read(uint8_t *buffer) {
	...
	__HAL_TIM_DISABLE_IT(modbus_io_htim, TIM_IT_CC2);
	...
	__HAL_TIM_ENABLE_IT(modbus_io_htim, TIM_IT_CC2);
...
void modbus_io_tim_handler(void) {
	if(__HAL_TIM_GET_FLAG(modbus_io_htim, TIM_FLAG_CC1))
		modbus_io_1_5_char_handler();
	else if(__HAL_TIM_GET_FLAG(modbus_io_htim, TIM_FLAG_CC2))
